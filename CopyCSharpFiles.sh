...
cp "$SRCROOT/src/PopMovieTexture.cs" $UNITY_PLUGINS_PATH
...
export GIT_REV_MATCH=POPMOVIE_GIT_REVISION

echo "Replacing $GIT_REV_MATCH in C# with $GIT_REV..."
# streamedit -inplace <backupextension> -e command
sed -i '' -e "s/$GIT_REV_MATCH/$GIT_REV/g" "$UNITY_PLUGINS_PATH/PopMovieTexture.cs"
if [ $? -ne 0 ]; then
{
